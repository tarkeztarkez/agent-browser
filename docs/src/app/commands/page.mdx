...
Use `tab new --window-id <id>` when connected to a multi-window Chrome instance and desktop focus must not determine the destination window. It requires an installed extension context that can call `chrome.tabs.create`. The exact new tab becomes agent-browser's active tab.

`tab ensure-window --window-id <id>` leaves the active tab unchanged when it is already in that native window. Otherwise it leaves the source tab untouched, copies its URL into a new tab in the requested window, and makes the copy active.
